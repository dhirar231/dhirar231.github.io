...
const configData = {
    web: {
        ...
        steps: [
            ...
            },
            {
                ...
                command:
                    "mysql -e \"CREATE DATABASE entreprise; CREATE USER 'webuser'@'localhost' IDENTIFIED BY 'SecurePass123!'; GRANT ALL ON entreprise.* TO 'webuser'@'localhost'; FLUSH PRIVILEGES;\"",
    ...
    monitoring: {
        title: "Configuration du serveur Prometheus & Grafana",
        subtitle: "Supervision moderne d’infrastructure (metrics & dashboards)",
        ...
        steps: [
            {
                ...
                command: "sudo apt update && sudo apt upgrade -y && sudo apt install -y wget curl gnupg2 software-properties-common",
                details: "Mettre à jour le système et installer les dépendances nécessaires",
            },
            {
                title: "Créer les utilisateurs système",
                command:
                    "sudo useradd --no-create-home --shell /bin/false prometheus && sudo useradd --no-create-home --shell /bin/false node_exporter",
                details: "Créer des comptes système sécurisés pour Prometheus et Node Exporter",
            },
            {
                title: "Installer Prometheus",
                command:
                    "wget https://github.com/prometheus/prometheus/releases/download/v2.51.0/prometheus-2.51.0.linux-amd64.tar.gz && tar xvf prometheus-*.tar.gz && sudo mv prometheus-*/prometheus /usr/local/bin/ && sudo mv prometheus-*/promtool /usr/local/bin/",
                details: "Télécharger et installer Prometheus",
            },
            {
                title: "Configurer Prometheus",
                command: "sudo mkdir -p /etc/prometheus /var/lib/prometheus && sudo mv prometheus-*/prometheus.yml /etc/prometheus/",
                details: "Créer les répertoires et installer le fichier de configuration",
            },
            {
                title: "Installer Node Exporter",
                command:
                    "wget https://github.com/prometheus/node_exporter/releases/download/v1.8.1/node_exporter-1.8.1.linux-amd64.tar.gz && tar xvf node_exporter-*.tar.gz && sudo mv node_exporter-*/node_exporter /usr/local/bin/",
                details: "Installer l’agent de collecte des métriques système",
            },
            {
                title: "Créer le service Prometheus",
                command:
                    "sudo tee /etc/systemd/system/prometheus.service > /dev/null <<EOF\n[Unit]\nDescription=Prometheus Monitoring\nAfter=network.target\n\n[Service]\nUser=prometheus\nExecStart=/usr/local/bin/prometheus --config.file=/etc/prometheus/prometheus.yml --storage.tsdb.path=/var/lib/prometheus\n\n[Install]\nWantedBy=multi-user.target\nEOF",
                details: "Créer le service systemd pour Prometheus",
            },
            {
                title: "Créer le service Node Exporter",
                command:
                    "sudo tee /etc/systemd/system/node_exporter.service > /dev/null <<EOF\n[Unit]\nDescription=Node Exporter\nAfter=network.target\n\n[Service]\nUser=node_exporter\nExecStart=/usr/local/bin/node_exporter\n\n[Install]\nWantedBy=multi-user.target\nEOF",
                details: "Créer le service systemd pour Node Exporter",
            },
            {
                title: "Installer Grafana",
                command:
                    'sudo apt install -y apt-transport-https && wget -q -O - https://packages.grafana.com/gpg.key | sudo apt-key add - && sudo add-apt-repository "deb https://packages.grafana.com/oss/deb stable main" && sudo apt update && sudo apt install -y grafana',
                details: "Installer Grafana depuis le dépôt officiel",
            },
            {
                title: "Démarrer & activer les services",
                command:
                    "sudo systemctl daemon-reexec && sudo systemctl daemon-reload && sudo systemctl enable --now prometheus node_exporter grafana-server",
                details: "Démarrer et activer tous les services au démarrage",
            },
        ],
        quickCommands: [
            {
                command: "sudo systemctl status prometheus",
                label: "Vérifier Prometheus",
            },
            {
                command: "sudo systemctl status grafana-server",
                label: "Vérifier Grafana",
            },
            {
                command: "curl http://localhost:9100/metrics",
                label: "Tester Node Exporter",
            },
        ],
        quickInfo: {
            ports: ["9090 (Prometheus)", "3000 (Grafana)", "9100 (Node Exporter)"],
            directories: ["/etc/prometheus", "/var/lib/prometheus", "/var/lib/grafana"],
            webInterface: {
                prometheus: "http://ip_du_serveur:9090",
                grafana: "http://ip_du_serveur:3000 (admin:admin)",
            },
        },
    },
    database: {
        ...
        steps: [
            ...
            },
            {
                ...
                command:
                    "mysql -e \"CREATE USER 'client'@'172.24.72.%' IDENTIFIED BY 'Client@Secure2025'; GRANT ALL PRIVILEGES ON *.* TO 'client'@'172.24.72.%' WITH GRANT OPTION; FLUSH PRIVILEGES;\"",
            ...
            },
            {
                ...
                command:
                    "mysql -e \"CREATE USER 'backup_user'@'localhost' IDENTIFIED BY 'Backup@123'; GRANT SELECT, RELOAD, PROCESS, LOCK TABLES ON *.* TO 'backup_user'@'localhost';\"",
            ...
            },
            {
                ...
                command:
                    'echo "0 2 * * * /usr/bin/mysqldump -u backup_user -pBackup@123 --all-databases | gzip > /backup/mysql-\\$(date +\\%Y\\%m\\%d).sql.gz" | sudo crontab -',
    ...
    nfs: {
        ...
        steps: [
            ...
            },
            {
                ...
                command:
                    'echo "/srv/nfs_collab 172.24.74.0/24(rw,sync,no_subtree_check,all_squash,anonuid=50000,anongid=50000)" | sudo tee -a /etc/exports',
...
function showConfig(serviceType) {
    ...
    stepsContent.innerHTML = service.steps
        .map(
            (step, index) => `
  ...
  `
    ...
    quickCommands.innerHTML = service.quickCommands
        .map(
            (cmd) => `
    <div class="col-12">
...
`;
    ...
    const stepsScript = service.steps
        .map(
            (step, index) => `
